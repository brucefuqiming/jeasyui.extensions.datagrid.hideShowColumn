...
* 注意：datagrid中最好不要设置fitColumns:true，自适应列在隐藏显示操作中不刷新页面的情况下可能导致列宽变窄、错位，处理方法参见“**自适应处理**”
        ...
        handler: function () {
          ...
          $.each(columnFields, function (i, columnField) {
            if ($.inArray(columnField, showColumns) != - 1) {
			  ...
			  //**自适应处理**对于设置了自适应列宽fitColumns=true的datagrid，为避免操作后造成表头与内容错位，暂时设置自动调整列宽以适应内容，刷新后重新自适应
			  if($datagrid.datagrid('options').fitColumns==true)
				$datagrid.datagrid('autoSizeColumn', columnField);
            } else {
		  ...
		  if(showColumns.length===0){
			...
			var hideShowColumn = JSON.parse(window.localStorage[storeHideShowDgField]);
	...
	toolbar:[
	  {
		...
		iconCls:'icon-back',
        handler: function () {
			$.messager.confirm('请谨慎操作', '确认放弃自定义的表头，恢复当前表格的隐藏/显示项到默认状态吗？', function(r){
				if (r){
					var storeHideShowDgField = $datagrid.datagrid('options').title+'_hideShowColumn';
					//window.localStorage.removeItem(storeHideShowDgField);
					//$.messager.show({title:'提示',msg:'已恢复默认，手动刷新当前页面后生效！'});
					$('#checkShowHideColumnDialog').dialog('close');
					var currentStoreHideShowDgField = JSON.parse(window.localStorage[storeHideShowDgField]);
					delete currentStoreHideShowDgField.hideColumn;
					delete currentStoreHideShowDgField.showColumn;
					window.localStorage[storeHideShowDgField] = JSON.stringify(currentStoreHideShowDgField);
					var allColumnFields = $datagrid.datagrid('getColumnFields');
					$.each(allColumnFields,function(i,columnField){
						if ($.inArray(columnField,currentStoreHideShowDgField.defaultHideColumn) != - 1)
							$datagrid.datagrid('hideColumn', columnField);
						else
							$datagrid.datagrid('showColumn', columnField);
						//**自适应处理**对于设置了自适应列宽fitColumns=true的datagrid，为避免操作后造成表头与内容错位，设置自动调整列宽以适应内容，刷新后重新自适应
						if($datagrid.datagrid('options').fitColumns==true)
							$datagrid.datagrid('autoSizeColumn', columnField);
					})
				}
      ...
      },
	  {
		...
		iconCls:'icon-redo',
        handler: function () {
			$.messager.confirm('请谨慎操作', '确认重置当前系统中所有表格隐藏/显示项的配置吗？', function(r){
				if (r){
...
function getHideShowDgColumnFromLocalStorage($datagrid){
  var gridTitle = $datagrid.datagrid('options').title;
  var storeHideShowDgField = gridTitle +'_hideShowColumn';
  //用于存储默认隐藏项的方法
  var saveDefaultHideColumn = function(){
		var allColumnFields = $datagrid.datagrid('getColumnFields');
		var defaultHideColumn = [];
		$.each(allColumnFields, function (i, columnField) {
          var fieldOptions = $datagrid.datagrid('getColumnOption', columnField);
          if (fieldOptions.hidden) {
            defaultHideColumn.push(columnField);
          }
        });
		var hideShowColumn = {'defaultHideColumn':''};
        hideShowColumn.defaultHideColumn = defaultHideColumn;
		window.localStorage[storeHideShowDgField] = JSON.stringify(hideShowColumn);
  }
  //仅对标题合法的datagrid进行操作
